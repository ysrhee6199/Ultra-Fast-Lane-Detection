...
def update_metrics(metric_dict, pair_data):
    ...
    for i in range(len(metric_dict['name'])):
        ...
        if predict is None:
            predict, target = converter(pair_data[data_src[0]]), converter(pair_data[data_src[1]])

        metric_op.update(predict, target)
        # metric_op.update(pair_data[data_src[0]], pair_data[data_src[1]])

        if (i+1) < len(metric_dict['name']) and (metric_dict['data_src'][i] != metric_dict['data_src'][i + 1]):
            predict, target = None, None

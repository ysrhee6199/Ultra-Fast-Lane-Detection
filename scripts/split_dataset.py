...
def split_dataset(file_in, test_split: int = 10, validate_split: int = 10):
    ...
    for i in range(resolution):
        ...
        test_list.extend(source_data[cur_line_no: int(cur_line_no + test_lines_count / resolution)])
        validate_list.extend(source_data[
                             int(cur_line_no + test_lines_count / resolution + 1)
                             : int(cur_line_no + test_lines_count / resolution + 1 + validate_lines_count / resolution)
                             ])
